...
export class HubClient {
    ...
    private getAuthHeaders(body: any) {
        ...
        const slug = process.env.GAME_SLUG || 'seven-up-down';
    ...
    async registerSelf(): Promise<any> {
        const slug = process.env.GAME_SLUG || 'seven-up-down';
        const serviceUrl = process.env.SERVICE_URL || 'http://localhost:3404';
        const wsUrl = process.env.WS_URL || 'ws://localhost:3404';
