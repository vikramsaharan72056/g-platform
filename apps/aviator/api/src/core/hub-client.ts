...
export class HubClient {
    ...
    private getAuthHeaders(body: any) {
        ...
        const slug = process.env.GAME_SLUG || 'aviator';
    ...
    /**
     * Verifies a player's token with the Hub
     */
    async verifyToken(token: string): Promise<any> {
        ...
        try {
            ...
            if (!res.ok) throw new UnauthorizedException('Token validation failed at Hub');
            return await res.json();
        } catch (e) {
            this.logger.error('Error verifying token with Hub', e);
            throw new UnauthorizedException('Auth unreachable');
        }
    }

    /**
     * Debits player wallet in the Hub
     */
    ...
    /**
     * Credits player wallet in the Hub
     */
    ...
    async registerSelf(): Promise<any> {
        const slug = process.env.GAME_SLUG || 'aviator';
        const serviceUrl = process.env.SERVICE_URL || 'http://localhost:3402';
        const wsUrl = process.env.WS_URL || 'ws://localhost:3402';
